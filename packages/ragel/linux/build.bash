...
BuildDependentPackage colm bin/colm
...
# --disable-manual is so as to avoid depending upon fig2dev and having to build it. We can add that later if we need it.
PrintRun ./configure --prefix="$INSTALL_DIR" --disable-manual
...
exeFile="$INSTALL_DIR/bin/ragel"
if [ ! -f "$exeFile" ]
then
  echo "ERROR: Could not find expected file at: $exeFile"
  exit 1
fi
